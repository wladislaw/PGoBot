...
class Settings(val properties: Properties) {
    ...
    val exitNoSpace = getPropertyIfSet("Exit if no space for new pokemon", "exit_if_no_space", true, String::toBoolean)
    ...
    val  maxTransferCP = getPropertyIfSet("Max CP to transfer", "max_transfer_cp" , 700, String::toInt)
    ...
    val  tryToEvolve = getPropertyIfSet("Try to evolve", "try_to_evolve", true, String::toBoolean)
    val pokemonToEvolve = if (tryToEvolve) {
        getPropertyIfSet("Pokemon to evolve", "pokemon_to_evolve", "CATERPIE,WEEDLE,PIDGEY,RATTATA,ZUBAT,VENONAT,DROWZEE,KRABBY",String::toString).split(",")
    } else {
        listOf()
    }
    val  minCandiesToEvolve = getPropertyIfSet("Minimum candies in candy jar after evolving", "min_candies_to_evolve", 40, String::toInt)
