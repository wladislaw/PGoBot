...
class CatchOneNearbyPokemon : Task {
    override fun run(bot: Bot, ctx: Context, settings: Settings) {
        ...
        if (pokemon.isNotEmpty()) {
            Log.green("Found ${pokemon.size} pokemon")
            var numberOfPokemonToCatch = pokemon.size
            Log.green("Going to catch ${pokemon.size} pokemon")
            for (catchablePokemon in pokemon) {
                catchPokemon(catchablePokemon, ctx, settings)
                numberOfPokemonToCatch--
            }
        }
    }

    fun catchPokemon(catchablePokemon: CatchablePokemon, ctx: Context, settings: Settings) {
